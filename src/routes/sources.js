...
router.get('/:id', async (req, res) => {
  try {
    ...
    logger.error('Failed to get source', { sourceId: req.params.id, error: error.message });
    res.status(500).json({
      success: false,
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (canonical_link_required !== undefined) updates.canonical_link_required = canonical_link_required;
    ...
    logger.error('Failed to update source', { sourceId: req.params.id, error: error.message });
    res.status(500).json({
      success: false,
...
router.get('/:id/stats', async (req, res) => {
  try {
    ...
    // Get latest article
    const latestArticles = await selectRecords('articles', 
      { source_id: sourceId }, 
      { 
    ...
    logger.error('Failed to get source stats', { sourceId: req.params.id, error: error.message });
    res.status(500).json({
      success: false,
