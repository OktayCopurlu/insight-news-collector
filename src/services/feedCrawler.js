...
export const crawlFeed = async (feed) => {
  ...
  try {
    ...
    for (const item of itemsToProcess) {
      try {
        const result = await processArticle(item, feed.source_id);
        if (result === null) {
          stats.skipped++;
...
export const crawlAllFeeds = async (options = {}) => {
  try {
    ...
    const perFeedLimit = options.perFeedLimit || null; // overrides env MAX_ITEMS_PER_RUN slice
    const totalLimit = options.totalLimit || null;
    let totalArticlesProcessed = 0;
    const results = { totalFeeds: feeds.length, successful: 0, failed: 0 };

    // Helper to wrap crawlFeed and count processed items (needs modification to know count)
    const runFeed = async (feed) => {
      try {
        // Crawl feed and then apply perFeedLimit if provided by slicing inside itemsToProcess logic.
        const stats = await crawlFeed(feed);
        // stats.processed includes how many articles attempted; adjust for perFeedLimit externally if needed
        let processedThisFeed = stats.processed;
        if (perFeedLimit !== null && processedThisFeed > perFeedLimit) {
          processedThisFeed = perFeedLimit; // approximate clamp
        }
        totalArticlesProcessed += processedThisFeed;
        results.successful++;
      } catch (error) {
        results.failed++;
    ...
    if (totalLimit) {
      // Sequential to respect total cap
      for (const feed of feeds) {
        if (totalLimit && totalArticlesProcessed >= totalLimit) break;
        await runFeed(feed);
        if (totalLimit && totalArticlesProcessed >= totalLimit) break;
      }
    } else {
      // Concurrent batches as before
      const batchSize = 5;
      for (let i = 0; i < feeds.length; i += batchSize) {
        const batch = feeds.slice(i, i + batchSize).map(runFeed);
        await Promise.all(batch);
      }
    }

    logger.info("All feeds crawl completed", {
      ...results,
      totalArticlesProcessed,
    });
    return { ...results, totalArticlesProcessed };
  } catch (error) {
...
export const getFeedStats = async (feedId) => {
  try {
    ...
    const logs = await selectRecords(
      "crawl_log",
      { feed_id: feedId },
      {
